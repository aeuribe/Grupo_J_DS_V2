...
public class UsuarioDao extends BaseDao<Usuario>{
    ...
    public Usuario getUsuarioByIdPersona( Persona persona )
    {
        Usuario result = EntityFactory.createUsuario();
        _logger.debug( String.format( "Get in UsuarioDao.getUsuarioByEmail: parameter {%s}", persona ) );
        try
        {
            CriteriaQuery<Usuario> query = _builder.createQuery( Usuario.class );
            Root<Usuario> root = query.from( Usuario.class );
            ...
            query.select( root );
            query.where( _builder.equal( root.get( "_persona" ), persona ) );
